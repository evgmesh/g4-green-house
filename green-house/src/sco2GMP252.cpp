...
sco2GMP252::sco2GMP252 (unsigned int retries, unsigned int wait)
    : node{ 240 }, co2_reg{ &node, 0x0000 }, co2_reg2{ &node, 0x0001 },
      voprcom_reg{ &node, 0x208 }, voprcom_reg2{ &node, 0x209 },
      vohumcom_reg{ &node, 0x20c }, vohumcom_reg2{ &node, 0x20d },
      mode_prescom_reg{ &node, 0x304 }, mode_tempcom_reg{ &node, 0x305 },
      mode_humcom_reg{ &node, 0x306 }, status_device_reg{ &node, 0x800 },
      status_co2_reg{ &node, 0x801 }, retries (retries), wait (wait),
      abspres_value (1015.0)
{
 ...
 * This function doesn't modify humidity and pressure values in volatile
 * registers.
 *
 * @param data value is set to the sensor reading on success. Not modified
 * otherwise.
 * @param retry flag to retry reading the sensor value while waiting
 * 'this->wait' ms between 'this->retries' readings.
 * @return true On success.
 * @return false On failure.
 */
bool
sco2GMP252::read_rapid (float &data, bool retry)
{
  ...
  for (unsigned int i = (retry ? 0 : this->retries);
       !this->status && i <= this->retries; ++i)
    {
      co2_hex = 0;
      temp = 0;
      ...
      if (!check_status ())
        {
          // If it is not the last attempt - wait for this->wait (100ms by
          // default) in case it was a minor failure.
          if (i != this->retries)
            vTaskDelay (this->wait);
          continue;
        }
  ...
  if (this->elapsed_time > start_time)
    this->elapsed_time -= start_time;
  else
    this->elapsed_time = 0xffffffff - start_time + 1 + this->elapsed_time;
  return this->status;
}

/**
 * @brief Reads sensor. Data might be imprecise.
 * This read() tries to modify humidity and pressure values in volatile
 * registers.
 *
 * @param data value is set to the sensor reading on success. Not modified
 * otherwise.
 ...
 */
bool
sco2GMP252::read (float &data, float rh, bool retry)
{
  unsigned int start_time = DWT->CYCCNT;
  unsigned int co2_hex;
  int temp;
  ...
  for (unsigned int i = (retry ? 0 : this->retries);
       !this->status && i <= this->retries; ++i)
    {
      ...
      if (!this->check_init_precise ())
        this->init_precise ();
      this->set_precise (abspres_value, rh);
 ...
 */
bool
sco2GMP252::read_precise (float &data, float rh, bool retry)
{
  unsigned int start_time = DWT->CYCCNT;
  unsigned int co2_hex;
  int temp;
  ...
  for (unsigned int i = (retry ? 0 : this->retries);
       !this->status && i <= this->retries; ++i)
    {
      ...
      if (!set_precise (abspres_value, rh))
        {
          // If it is not the last attempt - wait for this->wait (100ms by
          // default) in case it was a minor failure.
          if (i != this->retries)
 ...
 */
unsigned int
sco2GMP252::get_elapsed_time ()
{
  return this->elapsed_time;
}

/**
 * @brief Tries to setup precise sensor readings, with pres, temp and hum
 * registers.
 *
 ...
 */
int16_t
sco2GMP252::check_device_status ()
{
  vTaskDelay (5);
 ...
 */
bool
sco2GMP252::check_init_precise ()
{
  int check = 1;
  // If one of these registers returns -1 or 0 means, that sensor wasn't set up
  // earlier.
  vTaskDelay (5);
 ...
 */
bool
sco2GMP252::set_precise (float pres, float rh)
{
  bool res = true;
  int hex = 0;
  pres = abspres_value;
...
/**
 * @brief Convertion of 32-bit binary representation of decimal to float via
 * pointer.
 *
 ...
 */
float
sco2GMP252::binary32_to_float (const unsigned int bin32)
{
  return *((float *)&bin32);
}

/**
 * @brief Convertion of float to 32-bit binary representation of decimal via
 * pointer.
 *
 ...
 */
unsigned int
sco2GMP252::float_to_binary32 (float dec)
{
